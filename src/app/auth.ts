    ...
    providers: [
        Credentials({
            ...
            async authorize(credentials): Promise<ExtendedUser | null> {
                ...
                try {
                    const userRoleResponse = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL_V2_BACKEND}/v1/tp-users/user-info`, {
                        method: "GET",
                        headers: {
...
async function refreshAccessToken(token) {
    try {
        const url = `${process.env.NEXT_PUBLIC_API_BASE_URL_V2_BACKEND}/v1/auth/refresh-token`;
