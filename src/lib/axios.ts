...
const api = axios.create({
    baseURL: process.env.NEXT_PUBLIC_API_BASE_URL_V2_BACKEND,
    headers: {
...
const refreshAccessToken = async (refreshToken: string) => {
    try {
        const response = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL_V2_BACKEND}/v1/auth/refresh-token`, {
            grant_type: "refresh",
